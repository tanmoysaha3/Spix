...
internal class DatabaseIndexHelper
{
    ...
    internal async Task CreateSeriesIndex(SQLiteAsyncConnection connection)
    {
        await connection.ExecuteAsync(indexSeriesDetailsSeriesTypeQuery);
        await connection.ExecuteAsync(indexSeriesDetailsYearQuery);
        await connection.ExecuteAsync(indexSeriesDetailsRatingQuery);
        ...
        await connection.ExecuteAsync(indexSeriesTagsSeriesIdQuery);
        await connection.ExecuteAsync(indexSeriesTagsTagIdQuery);
    ...
    private readonly string indexSeriesDetailsRatingQuery = @"
        CREATE INDEX idx_SeriesDetails_Rating ON SeriesDetailsTable (Rating);";
    ...
    //SeriesTagsTable - for single column query
    private readonly string indexSeriesTagsSeriesIdQuery = @"
        CREATE INDEX idx_SeriesTags_SeriesId ON SeriesTagsTable (SeriesId);";
    private readonly string indexSeriesTagsTagIdQuery = @"
        CREATE INDEX idx_SeriesTags_TagId ON SeriesTagsTable (TagId);";
